...
inline bool compute_error(const std::array<double, 4>& abcd, const V3& point, const float length, scalar& error)
{
...
void SuperLIO::init(){
  ...
  if(g_save_map) point_map_.reset(new PointCloudType());
...
void SuperLIO::process(){
  if(!data_wrapper_->sync_measure(measures_)) return;
  (this->*state_fn_)();
}
...
void SuperLIO::caceData(){
  ...
  if(g_if_filter) pcl::transformPointCloud(*ds_undistort_, *world_pc_, transformation);
  else pcl::transformPointCloud(*scan_undistort_full_, *world_pc_, transformation);
  ...
  static bool pcd_dir_ready = false;
  if(!pcd_dir_ready){
    pcd_dir_ready = true;
    namespace fs = std::filesystem;
    std::error_code ec;
    fs::remove_all(g_save_map_dir + "/PCD", ec);
    fs::create_directories(g_save_map_dir + "/PCD", ec);
  }
  if (point_map_->size() > 0 && scan_wait_num >= g_pcd_save_interval) {
    pcd_index_++;
    std::string map_name(std::string(g_save_map_dir + "/PCD/scans_") + std::to_string(pcd_index_) + std::string(".pcd"));
...
void SuperLIO::ProcessCaceMap(){
  ...
  for (const auto& entry : fs::directory_iterator(pcd_folder)) {
    if (entry.path().extension() == ".pcd" && entry.path().filename().string().find("scans_") != std::string::npos) {
...
void SuperLIO::Output(){
  ...
  ExportScanDataset(scan_undistort_full_, state);
  Eigen::Matrix4f transformation = Eigen::Matrix4f::Identity();
  transformation.block<3, 3>(0, 0) = state.R.R_.cast<float>();
  transformation.block<3, 1>(0, 3) = state.p.cast<float>();
